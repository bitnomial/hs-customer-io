...
sendTransactionalEmail client templateId' recipient templateVars = do
    result <- runExceptT $ do
        let request = SendEmailRequest templateId' recipient templateVars Nothing
            authHeader = "Bearer " <> bearerToken (appClientAuth client)
        liftIO $ runClientM (sendEmailClient (Just authHeader) request) (appClientEnv client)
    case result of
        Left err -> return $ Left $ CustomerIOAppError err
